...
1. **NVIDIA NIM key** at build.nvidia.com (free credits) -- used by `supervisor.py`'s
   vision QA.
2. **CivitAI API key** at civitai.com/user/account -- read-only, fixes a region block
   on the search/gallery endpoints (model-version lookups and downloads work without it).
3. **TikTok developer app** at developers.tiktok.com: add "Login Kit" + "Content
...
4. **Verify the fetch domain** for Content Posting API's PULL_FROM_URL under the app's
...
5. Run `python3 get_tiktok_token.py` locally, **signed into the aibeauty TikTok
