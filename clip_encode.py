...
def encode(pipe, arch, prompt, negative):
    ...
    defeats the point. Falls back to a plain (truncated) call if compel is not
    installed OR raises at runtime -- a live CI run hit `'EmbeddingsProviderMulti'
    object has no attribute 'empty_z'` from compel 2.4.0's SDXL (multi-tokenizer)
    path, failing every single image for an SDXL checkpoint. That's a bug inside
    compel itself, not something this module can fix, so any failure here -- not
    just a missing import -- degrades to a truncated prompt instead of losing the
    whole generation.
    ...
    try:
        ...
        log(f"compel failed ({type(e).__name__}: {str(e)[:150]}); falling back to a "
           f"77-token-truncated prompt")
        return {"prompt": prompt, "negative_prompt": negative}
